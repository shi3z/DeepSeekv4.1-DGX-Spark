...
class ExpertStore:
    ...
    def _load_into_slot_inner(self, key: tuple, slot: int, prefix: str | None = None):
        ...
        # guard() raises if this expert is missing from the store AND its FP4 bytes were freed by
        # a100-vq/punch_fp4.py -- reading those would silently return zeros.
        if not hasattr(self, "cb3_store"):
            ...
            if self.cb3_store is not None:
                print(f"[a100-vq] pre-packed CB3 store: {len(self.cb3_store.records)} experts, "
                      f"{len(self.cb3_store.punched)} punched out of the checkpoint", flush=True)
        if self.cb3_store is not None:
            if self.cb3_store.has(key):
                return self.cb3_store.load(self.arena, slot, key, self._copy_stream())
            self.cb3_store.guard(key)
    ...
    def resolve(self, layer: int, experts: torch.Tensor, prefill: bool, split: bool = False,
        ...
        if to_load and split:
            ...
            ev.record()
            _cs.GATE = ev
            pool = self.spec_pool if spec else self.pool
