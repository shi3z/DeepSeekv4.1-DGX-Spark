...
* `tl.join`+`tl.reshape` vs `tl.interleave` for building the byte tile: identical results, both wrong
  end to end;
...
Put one nonzero weight in a known (row, k) with x = e_k and read which output element moves; that
localises a permutation in one run. The 8 `_chunk_dot` calls in `_vq_block_dot` are a direct copy of
CB3's, so the suspect is the pairing of `L_i`/`H_i` with `x_base + 32*i` and `s_i`.
